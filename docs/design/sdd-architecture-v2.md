...
|-----|------|--------|--------|
| ADR-001 | 未开始 | — | 全部 |
| ADR-002 | 已完成 | `ExecutionIntent` 退化为二进制 (static_flow/dynamic_flow)；`CapabilityTier` 4 值枚举；router 退化为纯粗筛（无本地数据→dynamic，否则→static，不分析查询意图） | — |
| ADR-003 | 基本完成 | `ExecutionStrategy` frozen=True；6 个 @computed_field 全部到位；Instructor pipeline 接入；`program_spec`/`repair_plan`/`resume_overlay` 从 ExecutionStrategy 删除；`analysis_plan: str` → `summary: str`；coder/debugger 不再覆写；registry 不再填充；旧 checkpoint migration validator；`ensure_artifact_plan`/`ensure_verification_plan` 删除 | Schema 层 `ExecutionStaticState`/`NodeOutputPatchState`/`DagGraphState` 上残留的旧顶级 key (program_spec/repair_plan/analysis_plan) |
| ADR-004 | 未开始 | — | 全部 |
...
> **已完成：** `CapabilityTier` 4 值枚举。`ExecutionIntent` 退化为二进制 (static_flow/dynamic_flow)。router 退化为纯粗筛（无本地数据→dynamic，否则→static，不分析查询意图）。analyst 作为 tier 唯一决策者。`analysis_mode` / `research_mode` 等作为 `@computed_field` 从 `capability_tier` 派生。`complexity_score` 从 `ExecutionIntent` 删除（router 不做复杂度分析，该字段无写入者）。
...
router 不产出 `CapabilityTier`（那是 analyst 的职责），只产出 `ExecutionIntent`：

```python
# src/common/contracts.py — ExecutionIntent 即路由决策
...
router_node:
  ...
  │     → intent="dynamic_flow", destinations=["dynamic_swarm"]
  │     → 跳过 analyst/coder/auditor，直接走 dynamic_node → summarizer
  │
  └── 其余一切（含"有本地数据"的情况）
        → intent="static_flow", destinations=["analyst"]
        → 走完整静态链（含 analyst）
        → analyst 在 ExecutionStrategy 中决定 capability_tier + fallback_tier
```

**router 判断逻辑**：只看一件事——有没有本地数据。

- 有本地数据（结构化数据集或业务文档）→ `static_flow`，走完整静态链（含 analyst），由 analyst 决定 capability_tier
- 无本地数据 → `dynamic_flow`，跳过整个静态链，直接走 dynamic_node
- 网络被禁 → 强制 `static_flow`

router 不分析查询意图、不判断时效性、不猜测知识域。"这个任务需要什么能力级别"完全交给 analyst。
...
pattern 列表保留但语义降级为"辅助粗筛判断的参考信号"，不再承担能力推断职责。router 只判断有无本地数据，不做任何查询意图分析。

#### Consequences

**变容易的事：**
- 要加新的路由行为只需在 router 里加一个分支，改 `ExecutionIntent.intent` + `destinations`
- 线上排查路由问题直接看 `ExecutionIntent.intent`，不再翻 pattern 匹配日志
...
- 能力级别判断完全由 analyst LLM 做出，router 不再参与
...
- 无本地数据直接走 dynamic 可能导致一些其实不需要动态探索的简单查询绕过 analyst（后果可控——dynamic 结束后仍会正常产出结果）
...
**1. ExecutionStrategy 重构：frozen=True + @computed_field** ✅ 已完成

> **已完成（2026-05-09）：** `frozen=True`。6 个 `@computed_field` 全部到位（`research_mode` / `strategy_family` / `generator_id` / `execution_intent` / `artifact_plan` / `verification_plan`）。`CapabilityTier` 4 值枚举。Instructor pipeline 接入 analyst_node。`program_spec` / `repair_plan` / `resume_overlay` 从 ExecutionStrategy 删除。`analysis_plan: str` → `summary: str`。coder / debugger 不再覆写 ExecutionStrategy。旧 checkpoint migration validator。
> **待完成：** Schema 层 `ExecutionStaticState` / `NodeOutputPatchState` / `DagGraphState` 上残留的旧顶级 key 清理。
...
| **删除** | `PreparedAnalysisPlan` dataclass | `analyst_node.py:22-27` | ✅ 已完成 |
| **删除** | `guidance_runner.py` | ADR-002 fine routing 移除后无用途 | 未完成 |
| **改为 @computed_field** | `analysis_mode`、`research_mode`、`strategy_family`、`generator_id` | `ExecutionStrategy` 内 | ✅ 已完成 |
| **改为 @computed_field** | `execution_intent`（原独立模型 `ExecutionIntent`） | `ExecutionStrategy` 内 | ✅ 已完成 |

**6. 旧字段兼容**

- `analysis_plan: str` → 删除。展示层读 `ExecutionStrategy.summary`。
...
拒绝了——`ExecutionStrategy.summary` 已承担展示角色。
...
当前 compiler 路径（`compile_static_program()`）仍是模板驱动：用写死的 if-else 生成 `ComputationStep` 列表 → 把 spec + payload 塞进 `_COMPILER_TEMPLATE` 字符串替换。
...
**4. Auditor 增强：Tree-sitter 替代 stdlib ast**
