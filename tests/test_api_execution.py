...
def _make_request(path: str, *, task_id: str | None = None, execution_id: str | None = None) -> Request:
    async def receive():
...
def test_list_task_executions_returns_sandbox_execution():
    ...
    response = asyncio.run(list_task_executions(_make_request(f"/api/tasks/{task_id}/executions", task_id=task_id)))
...
def test_list_execution_tool_calls_returns_static_sandbox_tool_calls():
    ...
    execution_blackboard.write(
        ...
        ExecutionData(
            ...
            task_envelope={"task_id": task_id, "tenant_id": tenant_id, "workspace_id": "ws-static-tool-call-api", "input_query": "summarize"},
            execution_record=ExecutionRecord(
    ...
    response = asyncio.run(
        list_execution_tool_calls(
            _make_request(f"/api/executions/{execution_id}/tool-calls", execution_id=execution_id)
        )
    )
    body = json.loads(response.body.decode())
...
def test_get_execution_returns_runtime_execution_and_artifacts():
    ...
    execution_blackboard.write(
        ...
        ExecutionData(
            ...
            dynamic_runtime_metadata={"effective_runtime_mode": "embedded", "sidecar_fallback_reason": "sidecar down"},
            dynamic_trace_refs=["trace-runtime"],
            dynamic_artifacts=["/tmp/runtime-report.md"],
            dynamic_trace=[
                {
    ...
    response = asyncio.run(get_execution(_make_request(f"/api/executions/{execution_id}", execution_id=execution_id)))
    body = json.loads(response.body.decode())
