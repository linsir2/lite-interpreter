    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                ...
                _check_assign_node(
                    node, var_module_mapping, var_method_mapping, trace_id
                )
...
def _check_getattr_call(
    ...
    if isinstance(target_node, ast.Name) and isinstance(attr_node, ast.Constant):
        ...
        raise AuditFailError(
            "沙箱环境禁止使用动态属性名的反射调用",
            trace_id,
            "dynamic_reflect_call_not_allowed"
        )
...
def _get_attribute_root_module(
    ...
    if depth > MAX_RECURSION_DEPTH:
        raise AuditFailError(
            "代码复杂度过高或涉嫌恶意混淆，超过AST最大解析深度",
            trace_id, 
            "ast_recursion_limit_exceeded"
        )
