...
from src.kag.compiler import LexiconMatcher, SpecCompiler
...
class ContextFormatter:
    ...
    def format(cls, candidates: list[dict[str, object]]) -> tuple[dict[str, list[str]], str]:
        ...
        for index, candidate in enumerate(candidates, start=1):
            ...
            lexical_signals = matcher.match_text(text)
            lexical_entities = [match.canonical for match in lexical_signals if match.category == "entity"]
            rule_spec = spec_compiler.parse_rule(text)
            metric_spec = spec_compiler.parse_metric(text)
            filter_spec = spec_compiler.parse_filter(text)
