...
class DocumentChunker:
    ...
    def _select_strategy(cls, sections: list[dict[str, Any]], content: str) -> ChunkingStrategy:
        ...
        if sections and len(sections) > 1:
            levels = [cls._section_level(section) for section in sections]
            section_kinds = {
                str((section.get("metadata") or {}).get("section_kind", "")).strip().lower() for section in sections
            }
            has_level_hierarchy = len(set(levels)) > 1 or any(level > 1 for level in levels)
            has_docling_structure = bool(section_kinds & {"document_title", "section_header"})
            if has_level_hierarchy or has_docling_structure:
    ...
    def _layout_aware_chunking(
        cls, sections: list[dict[str, Any]], doc_id: str, metadata: dict[str, Any]
    ) -> list[DocChunk]:
        """结构化感知分块：基于文档结构进行分块"""
        chunks = []
        ...
        for section in sections:
            ...
            section_level = cls._section_level(section)
            ...
            if section_level <= 2 and len(section_content) > PARENT_CHUNK_SIZE:
                ...
                for i, sub_content in enumerate(sub_chunks):
                    ...
                    chunk = DocChunk(
                        chunk_id=generate_uuid(), doc_id=doc_id, content=sub_content, metadata=chunk_metadata
                    )
                    chunks.append(chunk)
            else:
                ...
                chunk = DocChunk(
                    chunk_id=generate_uuid(), doc_id=doc_id, content=section_content, metadata=chunk_metadata
                )
                chunks.append(chunk)
            ...
            if len(section_content) > CHUNK_SIZE:
                ...
                for i, child_content in enumerate(child_chunks):
                    ...
                    child_chunk = DocChunk(
                        chunk_id=generate_uuid(), doc_id=doc_id, content=child_content, metadata=child_metadata
                    )
    ...
    def _sentence_splitter_chunking(cls, content: str, doc_id: str, metadata: dict[str, Any]) -> list[DocChunk]:
        ...
        for i, chunk_content in enumerate(sentence_chunks):
            ...
            chunk = DocChunk(chunk_id=generate_uuid(), doc_id=doc_id, content=chunk_content, metadata=chunk_metadata)
            chunks.append(chunk)
    ...
    @classmethod
    def _section_level(cls, section: dict[str, Any]) -> int:
